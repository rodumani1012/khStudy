	...
	<div id="content">
		<div class="inner">
			...
			<c:choose>
				...
				<c:otherwise>
					<h3>${user_info_dto.user_nickname }님 어서오세요 ٩(ˊᗜˋ*)و</h3>
				</c:otherwise>
			</c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${free }" var="free">
									<li><a href="home.do?command=board_detail&normal_num=${free.normal_num}">${free.normal_title }</a></li>
								</c:forEach>
							</c:otherwise>
						</c:choose>
					</ul>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${crew }" var="crew">
									<li><a href="home.do?command=board_detail&normal_num=${crew.normal_num}">${crew.normal_title }</a></li>
								</c:forEach>
							</c:otherwise>
						</c:choose>
					</ul>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${qa }" var="qa">
									<li><a href="home.do?command=board_detail&normal_num=${qa.normal_num}">${qa.normal_title }</a></li>
								</c:forEach>
							</c:otherwise>
						</c:choose>
					</ul>
