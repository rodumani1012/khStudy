								...
								<c:otherwise>
									<c:forEach items="${cal_list }" var="cal">
										...
										<c:forEach items="${prj_list }" var="prj">
											...
											<td><a href="${pageContext.request.contextPath }/calendar.do?command=issue_list_detail&issue_num=${cal.issue_num }&board_num=${cal.board_num}&year=${year}&month=${month}&date=${date}">${cal.issue_title }</a></td>
									<c:choose>
