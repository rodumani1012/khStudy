						...
						<tfoot>
							<tr>
								<td colspan="2">
									...
									<input type="hidden" id="normal_num" value="${normal_board_dto.normal_num }">
								</td>
					...
					<table>
						...
						<c:forEach items="${comment_board_dto }" var="item">
							<form action="home.do" onsubmit="comment_chk(${empty user_info_dto.user_num})">
								...
								<input type="hidden" name="comment_num" value="${item.comment_num}"> <input
									type="hidden" name="normal_num" value="${normal_board_dto.normal_num}">
								<tr>
									<th>
										<c:forEach begin="1" end="${item.comment_titletab }">
											RE:
										</c:forEach> ${item.user_nickname }
									</th>
									<td>${item.comment_content }</td>
									<td><a class="comment">&gt;&gt;답글</a> <textarea name="comment_comment" id=""
											cols="10" rows="3" style="display: none"></textarea> <input type="submit"
											value="작성" class="small" style="display: none">
		...
		$("#pay").click(function () {
			IMP.request_pay({
				...
				if (rsp.success) {
					...
					alert("결제에 성공하였습니다!");
					alert(normal_num);
					location.href = "home.do?command=board_detail&normal_num=" + normal_num;
				} else {
					msg = '결제에 실패하였습니다.';
