...
import com.toy.project.dto.issue_reply_dto;
import com.toy.project.dto.prj_in_user_dto;
...
public class project_daoimpl extends sql_map_config implements project_dao {
	...
	public int issue_update(Map<String, String> map) {
		...
		try {
			session = super.getSqlSessionFactory().openSession();
			res = session.update(namespace + "issue_" + map.get("column"), map);
		} catch (Exception e) {
			session.rollback();
			e.printStackTrace();
		} finally {
	...
	@Override
	public List<String> bring_user(int prj_num) {
		...
		list = session.selectList(namespace + "bring_user", prj_num);

		return list;
	}

	@Override
	public int issue_move(Map<String, String> map) {
		...
		try {
			session = super.getSqlSessionFactory().openSession();
			res = session.update(namespace + "issue_move", map);
		} catch (Exception e) {
			session.rollback();
			e.printStackTrace();
		} finally {
	...
	@Override
	public int board_update(Map<String, String> map) {
		...
		try {
			session = super.getSqlSessionFactory().openSession();
			res = session.update(namespace+"board_update", map);
		} catch (Exception e) {
			session.rollback();
			e.printStackTrace();
		} finally {
	...
	@Override
	public prj_info_dto bring_prj_name(int prj_num, int user_num) {
		System.out.println("프로젝트 번호 가져오기");
		...
		session = getSqlSessionFactory().openSession();
		dto = session.selectOne(namespace + "bring_prj_name", map);
	...
	@Override
	public prj_in_user_dto pick_user(int prj_num, int user_num) {
		System.out.println("해당 프로젝트에 로그인된 유저가 들어가있는지 확인");
		prj_in_user_dto dto = new prj_in_user_dto();

		Map<String, Integer> map = new HashMap<String, Integer>();
		map.put("prj_num", prj_num);
		map.put("user_num", user_num);

		System.out.println("pick유저:" + map);

		SqlSession session = null;

		session = getSqlSessionFactory().openSession();

		dto = session.selectOne(namespace + "pick_user", map);

		return dto;
	}

	@Override
	public prj_info_dto pick_user_nick(int prj_num) {
		System.out.println("프로젝트 작성자 가져오기");
		prj_info_dto dto = new prj_info_dto();
		...
		session = getSqlSessionFactory().openSession();
		dto = session.selectOne(namespace + "pick_user_nick", map);
	...
	@Override
	public int reply_insert(issue_reply_dto dto) {
		...
		try {
			session = super.getSqlSessionFactory().openSession();
			res = session.insert(namespace + "reply_insert", dto);
		} catch (Exception e) {
			session.rollback();
			e.printStackTrace();
		} finally {
	...
	@Override
	public List<String> bring_reply(int issue_num) {
		
		SqlSession session = null;
		
		session = super.getSqlSessionFactory().openSession();
		List<String> list = session.selectList(namespace+"bring_reply", issue_num);
		
		return list;
	}

	@Override
	public List<String> prj_search(String select, String keyword) {
		...
		session = super.getSqlSessionFactory().openSession();
		List<String> list = session.selectList(namespace+"prj_search_"+select, map);
